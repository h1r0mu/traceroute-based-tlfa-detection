...
for env in env_list:
  ...
  for l in lambdas:
    ...
    txt = ''
    if (env == MyConf.ENV_CAIDA_X100):
      txt = '$t_{traceroute} =$ 5 minutes'
    if (env == MyConf.ENV_CAIDA_X10):
      txt = '$t_{traceroute} =$ 50 minutes'
    if (env == MyConf.ENV_CAIDA_ORIGINAL):
      txt = '$t_{traceroute} =$ 500 minutes'
    plt.text(3500, 0.2, txt, ha='center', va='top')
...
# bounding box adjusted!!
    ...
    outputFileName = "./image/auc_vs_degree/%s_lambda%09d_with_text.eps" % (topology, l)

...
print yc
for a in lambdalist:
    filename = '../AUC/dataset1000_5ms/AUC2/lambda%09d.txt'%(a)
    for line in open(filename,'r'):
        para = line.split(',')
        AS = int(para[0])
        auc = float(para[1])
...
ax.set_xlabel('$\lambda$')         # X tick label
ax.set_ylabel('Number of ASes (AUC > 0.9)')                  # Y tick label
ax.set_ylim([0, 700])            # [min, max] value of Y tick
...
plt.savefig('./image/ASvsnoise_AS100.eps',
    dpi=300,
    transparent=True,     
